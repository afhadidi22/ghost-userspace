...
// Default weight suggestions – tune freely.
constexpr double PRIO_BOOST_WEIGHT        = 1'000.0;
constexpr double PREEMPTED_WEIGHT         =   200.0;
constexpr double QUEUE_POSITION_WEIGHT    =    10.0;

inline double ScoreTask(FifoTask* task,
                        size_t queue_position) {
  double score = 0.0;

  if (task->prio_boost)      score += PRIO_BOOST_WEIGHT;
  if (task->preempted)       score += PREEMPTED_WEIGHT;

  score += QUEUE_POSITION_WEIGHT / static_cast<double>(queue_position + 1);
...
FifoTask* FifoRq::Dequeue() {
  ...
  FifoTask* best = rq_[best_idx];
  rq_.erase(rq_.cbegin() + best_idx);   // pop it out
  best->run_state = FifoTaskState::kRunnable;
  return best;
}
